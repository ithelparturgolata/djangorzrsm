...
if($_POST)
{
    ...
    $your_email = "artur.golata@smbudowlani.pl";
...
//    $mail->Port       = 587;                                    // TCP port to connect to
    ...
    $mail->Subject = 'Panel Mieszkanca';
    $mail->Body  = "<h4 style='text-align: center;padding: 25px 15px;background-color: #0c6c9e;color: #FFFFFF;font-size:16px;width:90%;border-radius: 10px;'>Cześć masz nową wiadomość z Panelu Mieszkańca</h4><br><br>";

    if(isset($_POST["userEmail"])) {
        $mail->Body .= "<strong>Adres email: </strong>" . $user_Email . "<br>";
    }
    //education
    if(isset($_POST["fatherName"])) {
    ...
    if(isset($_POST["userAddress"])) {
        $mail->Body .= "<strong>Adres: </strong>" . $user_Address . "<br>";
    }
    if(isset($_POST["course"])) {
        $mail->Body .= "<strong>Applied Course: </strong>" . $applied_Course . "<br>";
    }
    if(isset($_POST["userPhone"])) {
        $mail->Body .= "<strong>Telefon: </strong>" . $user_Phone . "<br>";
    }
    if(isset($_POST["userSubject"])) {
        $mail->Body .= "<strong>Temat: </strong>" . $user_Subject . "<br>";
    }
    if(isset($_POST["userCity"])) {
    ...
    if(isset($_POST["userMessage"])) {
        $mail->Body .= "<strong>Wiadomość: </strong><br><br><div style='background-color: #EDEFF2;padding:30px 15px;border-radius:10px;min-height:50px;width:90%;'>" . $user_Message . "</div><br>";
    }
    $mail->Body .= '<strong>Z poważaniem,</strong><br>';
    ...
    if(!$mail->send())
    {
        ...
        $output = json_encode(array('type'=>'message', 'text' => 'Cześć '.$user_Name .' Dziękujemy za kontakt z nami'));
        die($output);
    }
}
?>
